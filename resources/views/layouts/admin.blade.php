...
<body class="h-full font-body antialiased bg-gray-50 text-gray-800" x-data="{ sidebarOpen: true, mobileSidebar: false }">
        ...
        <aside class="hidden lg:flex lg:flex-col bg-ris-dark-900 text-white transition-all duration-300 ease-in-out"
                ...
                @foreach ($sidebarItems as $item)
                    <a href="{{ route($item['route']) }}"
                       ...
                       {{ request()->routeIs($item['route']) ? 'bg-ris-primary text-white shadow-md' : 'text-gray-300 hover:bg-white/10 hover:text-white' }}">
                        <svg class="w-5 h-5 shrink-0 {{ request()->routeIs($item['route']) ? 'text-white' : 'text-gray-400 group-hover:text-white' }}"
            ...
            <div class="border-t border-white/10 p-3 shrink-0">
                <button @click="sidebarOpen = !sidebarOpen"
                    class="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-gray-400 hover:bg-white/10 hover:text-white transition-colors text-sm">
