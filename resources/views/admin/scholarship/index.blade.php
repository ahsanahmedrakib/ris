                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}"
                                placeholder="নাম বা রেজি নং দিয়ে খুঁজুন..."
                    ...
                    <div>
                        ...
                        <select name="class_no"
                            ...
                            @foreach ($classes as $value => $label)
                                <option value="{{ $value }}" {{ request('class_no') == $value ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        @forelse($registrations as $index => $registration)
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-4 py-3 text-gray-500 whitespace-nowrap">
                                    {{ ($registrations->currentPage() - 1) * $registrations->perPage() + $index + 1 }}</td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <button @click="openViewModal({{ $registration->id }})"
                                        class="font-heading font-semibold text-ris-primary hover:underline cursor-pointer">{{ $registration->registration_no }}</button>
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        <div
                                ...
                                <td class="px-4 py-3 text-gray-600 whitespace-nowrap">
                                    {{ $classes[$registration->class_no] ?? '-' }}</td>
                                <td class="px-4 py-3 text-gray-600 whitespace-nowrap">{{ $registration->roll_no ?? '-' }}
                                </td>
                                    ...
                                    <form method="POST" action="{{ route('admin.scholarship.status', $registration) }}"
                                        ...
                                        <select name="status" x-model="status" @change="$el.form.submit()"
                                            ...
                                            <option value="approved"
                                                {{ $registration->status === 'approved' ? 'selected' : '' }}>অ্যাকসেপ্টেড
                                            </option>
                                            <option value="rejected"
                                                {{ $registration->status === 'rejected' ? 'selected' : '' }}>রিজেক্টেড
                                            </option>
                                        </select>
                                    </form>
                                </td>
                                ...
                                <td class="px-4 py-3 sticky right-0 bg-white z-10">
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        <button @click="openEditModal({{ $registration->id }})"
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="px-5 py-12 text-center">
                                    ...
                                    <p class="text-sm text-gray-400 mt-1">নতুন রেজিস্ট্রেশন করুন বা ফিল্টার পরিবর্তন করুন
                                    </p>
                                </td>
                            </tr>
                        @endforelse
                ...
                <form action="{{ route('admin.scholarship.store') }}" method="POST" class="p-6 space-y-5"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="student_name" x-model="createForm.student_name"
                                @blur="validateCreateField('student_name')"
                                ...
                                :class="(createErrors.student_name || (createAttempted && !createForm.student_name)) ?
                                'border-red-400' : 'border-gray-200'">
                            <template x-if="createErrors.student_name || (createAttempted && !createForm.student_name)">
                                <p class="mt-1 text-xs text-red-600"
                                    x-text="createErrors.student_name || 'শিক্ষার্থীর নাম আবশ্যক'"></p>
                            </template>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">পিতার নাম <span
                                    class="text-red-500">*</span></label>
                            <input type="text" name="father_name" x-model="createForm.father_name"
                                @blur="validateCreateField('father_name')"
                                class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                                :class="(createErrors.father_name || (createAttempted && !createForm.father_name)) ?
                                'border-red-400' : 'border-gray-200'">
                            <template x-if="createErrors.father_name || (createAttempted && !createForm.father_name)">
                                <p class="mt-1 text-xs text-red-600"
                                    x-text="createErrors.father_name || 'পিতার নাম আবশ্যক'"></p>
                            </template>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">মাতার নাম <span
                                    class="text-red-500">*</span></label>
                            <input type="text" name="mother_name" x-model="createForm.mother_name"
                                @blur="validateCreateField('mother_name')"
                                class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                                :class="(createErrors.mother_name || (createAttempted && !createForm.mother_name)) ?
                                'border-red-400' : 'border-gray-200'">
                            <template x-if="createErrors.mother_name || (createAttempted && !createForm.mother_name)">
                                <p class="mt-1 text-xs text-red-600"
                                    x-text="createErrors.mother_name || 'মাতার নাম আবশ্যক'"></p>
                            </template>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">স্কুলের নাম <span
                                    class="text-red-500">*</span></label>
                            <input type="text" name="school_name" x-model="createForm.school_name"
                                @blur="validateCreateField('school_name')"
                                class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                                :class="(createErrors.school_name || (createAttempted && !createForm.school_name)) ?
                                'border-red-400' : 'border-gray-200'">
                            <template x-if="createErrors.school_name || (createAttempted && !createForm.school_name)">
                                <p class="mt-1 text-xs text-red-600"
                                    x-text="createErrors.school_name || 'স্কুলের নাম আবশ্যক'"></p>
                            </template>
                        </div>
                        <div>
                            ...
                            <template x-if="createErrors.class_no || (createAttempted && !createForm.class_no)">
                                <p class="mt-1 text-xs text-red-600" x-text="createErrors.class_no || 'শ্রেণি আবশ্যক'">
                                </p>
                            </template>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">রোল নং <span
                                    class="text-red-500">*</span></label>
                            <input type="text" name="roll_no" x-model="createForm.roll_no"
                                @blur="validateCreateField('roll_no')"
                                class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                                :class="(createErrors.roll_no || (createAttempted && !createForm.roll_no)) ? 'border-red-400' :
                                'border-gray-200'">
                            <template x-if="createErrors.roll_no || (createAttempted && !createForm.roll_no)">
                                <p class="mt-1 text-xs text-red-600" x-text="createErrors.roll_no || 'রোল নং আবশ্যক'"></p>
                            </template>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">মোবাইল নং <span
                                    class="text-red-500">*</span></label>
                            <input type="tel" name="mobile_no" x-model="createForm.mobile_no"
                                @blur="validateCreateField('mobile_no')"
                                class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                                :class="(createErrors.mobile_no || (createAttempted && !createForm.mobile_no)) ?
                                'border-red-400' : 'border-gray-200'">
                            <template x-if="createErrors.mobile_no || (createAttempted && !createForm.mobile_no)">
                                <p class="mt-1 text-xs text-red-600"
                                    x-text="createErrors.mobile_no || 'মোবাইল নং আবশ্যক'"></p>
                            </template>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">পেমেন্ট মাধ্যম <span
                                    class="text-red-500">*</span></label>
                            <select name="payment_method" x-model="createForm.payment_method"
                                @change="validateCreateField('payment_method'); validateCreateField('bkash_no')"
                                class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors bg-white"
                                :class="createErrors.payment_method ? 'border-red-400' : 'border-gray-200'">
                                <option value="bkash">বিকাশ</option>
                                <option value="cash">ক্যাশ</option>
                            </select>
                            <template x-if="createErrors.payment_method">
                                <p class="mt-1 text-xs text-red-600" x-text="createErrors.payment_method"></p>
                            </template>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">বিকাশ নম্বর <template
                                    x-if="createForm.payment_method === 'bkash'"><span
                                        class="text-red-500">*</span></template></label>
                            <input type="tel" name="bkash_no" x-model="createForm.bkash_no"
                                @blur="validateCreateField('bkash_no')"
                                class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                                :class="(createErrors.bkash_no || (createAttempted && createForm
                                    .payment_method === 'bkash' && !createForm.bkash_no)) ? 'border-red-400' :
                                'border-gray-200'">
                            <template
                                x-if="createErrors.bkash_no || (createAttempted && createForm.payment_method === 'bkash' && !createForm.bkash_no)">
                                <p class="mt-1 text-xs text-red-600"
                                    x-text="createErrors.bkash_no || 'বিকাশ নম্বর আবশ্যক'"></p>
                            </template>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">স্ট্যাটাস <span
                                    class="text-red-500">*</span></label>
                            <select name="status" x-model="createForm.status" @change="validateCreateField('status')"
                                class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors bg-white"
                                :class="createErrors.status ? 'border-red-400' : 'border-gray-200'">
                                <option value="pending">পেন্ডিং</option>
                                <option value="approved">অ্যাকসেপ্টেড</option>
                                <option value="rejected">রিজেক্টেড</option>
                            </select>
                            <template x-if="createErrors.status">
                                <p class="mt-1 text-xs text-red-600" x-text="createErrors.status"></p>
                            </template>
                        </div>
                    </div>
                    ...
                    <template x-if="viewData">
                        <div>
                            <div class="text-center mb-5">
                                <span
                                    class="inline-flex items-center px-4 py-2 bg-ris-primary/10 border border-ris-primary/20 rounded-lg font-heading font-bold text-ris-primary tracking-wider"
                                    x-text="viewData.registration_no"></span>
                            </div>
                            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 text-sm">
                                ...
                                <div>
                                    ...
                                    <dd class="font-medium text-gray-900"
                                        x-text="viewData.payment_method === 'cash' ? 'ক্যাশ' : 'বিকাশ'"></dd>
                                </div>
            ...
            <div
                ...
                <div class="p-6">
                    <template x-if="editData">
                        <form :action="'{{ url('admin/scholarship') }}/' + editData.id" method="POST" class="space-y-5"
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">শিক্ষার্থীর নাম <span
                                            class="text-red-500">*</span></label>
                                    <input type="text" name="student_name" x-model="editData.student_name"
                                        @blur="validateEditField('student_name')"
                                        class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                                        :class="(editErrors.student_name || (editAttempted && !editData.student_name)) ?
                                        'border-red-400' : 'border-gray-200'">
                                    <template x-if="editErrors.student_name || (editAttempted && !editData.student_name)">
                                        <p class="mt-1 text-xs text-red-600"
                                            x-text="editErrors.student_name || 'শিক্ষার্থীর নাম আবশ্যক'"></p>
                                    </template>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">পিতার নাম <span
                                            class="text-red-500">*</span></label>
                                    <input type="text" name="father_name" x-model="editData.father_name"
                                        @blur="validateEditField('father_name')"
                                        class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                                        :class="(editErrors.father_name || (editAttempted && !editData.father_name)) ?
                                        'border-red-400' : 'border-gray-200'">
                                    <template x-if="editErrors.father_name || (editAttempted && !editData.father_name)">
                                        <p class="mt-1 text-xs text-red-600"
                                            x-text="editErrors.father_name || 'পিতার নাম আবশ্যক'"></p>
                                    </template>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">মাতার নাম <span
                                            class="text-red-500">*</span></label>
                                    <input type="text" name="mother_name" x-model="editData.mother_name"
                                        @blur="validateEditField('mother_name')"
                                        class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                                        :class="(editErrors.mother_name || (editAttempted && !editData.mother_name)) ?
                                        'border-red-400' : 'border-gray-200'">
                                    <template x-if="editErrors.mother_name || (editAttempted && !editData.mother_name)">
                                        <p class="mt-1 text-xs text-red-600"
                                            x-text="editErrors.mother_name || 'মাতার নাম আবশ্যক'"></p>
                                    </template>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">স্কুলের নাম <span
                                            class="text-red-500">*</span></label>
                                    <input type="text" name="school_name" x-model="editData.school_name"
                                        @blur="validateEditField('school_name')"
                                        class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                                        :class="(editErrors.school_name || (editAttempted && !editData.school_name)) ?
                                        'border-red-400' : 'border-gray-200'">
                                    <template x-if="editErrors.school_name || (editAttempted && !editData.school_name)">
                                        <p class="mt-1 text-xs text-red-600"
                                            x-text="editErrors.school_name || 'স্কুলের নাম আবশ্যক'"></p>
                                    </template>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">শ্রেণি <span
                                            class="text-red-500">*</span></label>
                                    <select name="class_no" x-model="editData.class_no"
                                        @change="validateEditField('class_no')"
                                        class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors bg-white"
                                        :class="(editErrors.class_no || (editAttempted && !editData.class_no)) ?
                                        'border-red-400' : 'border-gray-200'">
                                        <option value="">-- শ্রেণি নির্বাচন --</option>
                                        @foreach ($classes as $value => $label)
                                            <option value="{{ $value }}">{{ $label }}</option>
                                    ...
                                    <template x-if="editErrors.class_no || (editAttempted && !editData.class_no)">
                                        <p class="mt-1 text-xs text-red-600"
                                            x-text="editErrors.class_no || 'শ্রেণি আবশ্যক'"></p>
                                    </template>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">রোল নং <span
                                            class="text-red-500">*</span></label>
                                    <input type="text" name="roll_no" x-model="editData.roll_no"
                                        @blur="validateEditField('roll_no')"
                                        class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                                        :class="(editErrors.roll_no || (editAttempted && !editData.roll_no)) ?
                                        'border-red-400' : 'border-gray-200'">
                                    <template x-if="editErrors.roll_no || (editAttempted && !editData.roll_no)">
                                        <p class="mt-1 text-xs text-red-600"
                                            x-text="editErrors.roll_no || 'রোল নং আবশ্যক'"></p>
                                    </template>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">মোবাইল নং <span
                                    ...
                                    <input type="tel" name="mobile_no" x-model="editData.mobile_no"
                                        ...
                                        class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                                        :class="(editErrors.mobile_no || (editAttempted && !editData.mobile_no)) ?
                                        'border-red-400' : 'border-gray-200'">
                                    <template x-if="editErrors.mobile_no || (editAttempted && !editData.mobile_no)">
                                        <p class="mt-1 text-xs text-red-600"
                                            x-text="editErrors.mobile_no || 'মোবাইল নং আবশ্যক'"></p>
                                    </template>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">পেমেন্ট মাধ্যম <span
                                    ...
                                    <select name="payment_method" x-model="editData.payment_method"
                                        @change="validateEditField('payment_method'); validateEditField('bkash_no')"
                                        class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors bg-white"
                                        :class="editErrors.payment_method ? 'border-red-400' : 'border-gray-200'">
                                        <option value="bkash">বিকাশ</option>
                                        <option value="cash">ক্যাশ</option>
                                    </select>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">বিকাশ নম্বর <template
                                            x-if="editData.payment_method === 'bkash'"><span
                                                class="text-red-500">*</span></template></label>
                                    <input type="tel" name="bkash_no" x-model="editData.bkash_no"
                                        @blur="validateEditField('bkash_no')"
                                        class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                                        :class="(editErrors.bkash_no || (editAttempted && editData
                                            .payment_method === 'bkash' && !editData.bkash_no)) ? 'border-red-400' :
                                        'border-gray-200'">
                                    <template
                                        x-if="editErrors.bkash_no || (editAttempted && editData.payment_method === 'bkash' && !editData.bkash_no)">
                                        <p class="mt-1 text-xs text-red-600"
                                            x-text="editErrors.bkash_no || 'বিকাশ নম্বর আবশ্যক'"></p>
                                    </template>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">স্ট্যাটাস <span
                                    ...
                                    <select name="status" x-model="editData.status"
                                        ...
                                        class="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors bg-white"
                                        :class="editErrors.status ? 'border-red-400' : 'border-gray-200'">
                                        <option value="pending">পেন্ডিং</option>
                                        <option value="approved">অ্যাকসেপ্টেড</option>
                                        <option value="rejected">রিজেক্টেড</option>
                                    </select>
                                    <template x-if="editErrors.status">
                                        <p class="mt-1 text-xs text-red-600" x-text="editErrors.status"></p>
                                    </template>
                                </div>
                            </div>
                            <div class="flex justify-end gap-3 pt-2">
            ...
            function scholarshipApp() {
                return {
                    ...
                    validateField(field, data, errors) {
                        ...
                        if (['student_name', 'father_name', 'mother_name', 'school_name', 'class_no', 'roll_no', 'mobile_no',
                                'payment_method', 'status'
                            ].includes(field)) {
                            if (!val || val.trim() === '') {
                                const msgs = {
                                    student_name: 'শিক্ষার্থীর নাম আবশ্যক।',
                                    father_name: 'পিতার নাম আবশ্যক।',
                                    mother_name: 'মাতার নাম আবশ্যক।',
                                    school_name: 'স্কুলের নাম আবশ্যক।',
                                    class_no: 'শ্রেণি নির্বাচন আবশ্যক।',
                                    roll_no: 'রোল নং আবশ্যক।',
                                    mobile_no: 'মোবাইল নম্বর আবশ্যক।',
                                    payment_method: 'পেমেন্ট মাধ্যম নির্বাচন করুন।',
                    ...
                    validateCreateForm(el) {
                        ...
                        this.createErrors = {};
                        let valid = true;
                        ['student_name', 'father_name', 'mother_name', 'school_name', 'class_no', 'roll_no', 'mobile_no',
                            'payment_method', 'bkash_no', 'status'
                        ].forEach(f => {
