...
@section('title', 'একাডেমিক ক্যালেন্ডার তালিকা')
...
@section('content')
    <div class="space-y-6" x-data="calendarApp()">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-heading font-bold text-gray-900">একাডেমিক ক্যালেন্ডার</h1>
                <p class="text-sm text-gray-500 mt-1">একাডেমিক ক্যালেন্ডার ফাইল পরিচালনা করুন</p>
            </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <form method="GET" action="{{ route('admin.academic-calendars.index') }}">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">বছর</label>
                        <select name="year"
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="gradient-logo">
                            ...
                            <th class="text-left px-4 py-3.5 font-medium text-white whitespace-nowrap">বছর</th>
                            <th class="text-left px-4 py-3.5 font-medium text-white whitespace-nowrap">ফাইল</th>
                    ...
                    <tbody class="divide-y divide-gray-50" data-table-body>
                        @forelse($calendars as $calendar)
                            <tr class="hover:bg-gray-50 transition-colors"
                                x-data="activeRow('{{ url('admin/academic-calendars') }}', {{ $calendar->id }}, {{ $calendar->is_active ? 'true' : 'false' }})">
                                <td class="px-4 py-3">
                                    <button @click="openViewModal({{ $calendar->id }})"
                                        class="font-heading font-semibold text-ris-primary hover:underline cursor-pointer">{{ $calendar->title }}</button>
                                </td>
                                <td class="px-4 py-3 text-gray-600 whitespace-nowrap">{{ $calendar->year ?? '-' }}</td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        <span class="px-2.5 py-1 rounded-lg bg-red-50 text-red-600 text-xs font-semibold">PDF</span>
                                        <span class="text-gray-500 truncate max-w-40">{{ $calendar->file_name }}</span>
                                    </div>
                                </td>
                                <td class="px-4 py-3 text-center whitespace-nowrap">
                                    <span class="px-2.5 py-1 rounded-full text-xs font-medium"
                                            ...
                                            x-text="active ? 'সক্রিয়' : 'নিষ্ক্রিয়'">{{ $calendar->is_active ? 'সক্রিয়' : 'নিষ্ক্রিয়' }}</span>
                                </td>
                                <td class="px-4 py-3 sticky right-0 bg-white z-10">
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        <button @click="openEditModal({{ $calendar->id }})"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-5 py-12 text-center">
                                    ...
                                    <p class="text-gray-500 font-medium">কোনো ক্যালেন্ডার পাওয়া যায়নি</p>
                                    <p class="text-sm text-gray-400 mt-1">নতুন ক্যালেন্ডার আপলোড করুন বা ফিল্টার পরিবর্তন করুন</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            @include('vendor.pagination.custom', ['paginator' => $calendars])
        </div>
        ...
        <div x-show="showCreateModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div
                ...
                <div class="gradient-logo px-6 py-4 flex items-center justify-between rounded-t-2xl">
                    <h3 class="font-heading font-bold text-white text-lg">নতুন একাডেমিক ক্যালেন্ডার</h3>
                ...
                <form action="{{ route('admin.academic-calendars.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="title" x-model="createForm.title"
                                ...
                                'border-gray-200'"
                                placeholder="যেমন: একাডেমিক ক্যালেন্ডার ২০২৬">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="description" rows="3" x-model="createForm.description"
                                ...
                                placeholder="ক্যালেন্ডারের সংক্ষিপ্ত বিবরণ (ঐচ্ছিক)..."></textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">বছর</label>
                            <input type="text" name="year" x-model="createForm.year"
                        ...
                        <div class="sm:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">পিডিএফ ফাইল <span
                                    class="text-red-500">*</span></label>
                            <input type="file" name="file" accept="application/pdf"
                                ...
                                :class="createErrors.file ? 'border-red-400' : 'border-gray-200'">
                            <template x-if="createErrors.file">
                                <p class="mt-1 text-xs text-red-600" x-text="createErrors.file"></p>
                            </template>
                            <p class="mt-1 text-xs text-gray-400">শুধুমাত্র PDF — সর্বোচ্চ 10MB</p>
                        </div>
                        <div class="sm:col-span-2">
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                        ...
                        <button type="submit"
                            class="px-6 py-2.5 bg-ris-primary text-white text-sm font-medium rounded-lg hover:bg-ris-dark transition-colors shadow-sm cursor-pointer">আপলোড
                            করুন</button>
                    </div>
                </form>
            </div>
        ...
        <div x-show="showViewModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div
                class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-slide-up">
                <div class="gradient-logo px-6 py-4 flex items-center justify-between rounded-t-2xl">
                    <h3 class="font-heading font-bold text-white text-lg">ক্যালেন্ডারের তথ্য</h3>
                    ...
                    <template x-if="viewData">
                        <div>
                            <div class="mb-5 flex items-center gap-4">
                                <div class="w-16 h-16 rounded-xl flex flex-col items-center justify-center border-2 border-red-200 bg-red-50">
                                    <span class="text-red-600 font-heading font-bold text-lg">PDF</span>
                                </div>
                                <div>
                                    <h4 class="font-heading font-bold text-lg text-gray-900" x-text="viewData.title"></h4>
                                    <p class="text-sm text-gray-500" x-text="viewData.file_name"></p>
                                </div>
                            </div>
                            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 text-sm">
                                <div class="sm:col-span-2" x-show="viewData.description">
                                    <dt class="text-gray-500 mb-1">বিবরণ</dt>
                                    <dd class="font-medium text-gray-900" x-text="viewData.description"></dd>
                                </div>
                                <div>
                                    <dt class="text-gray-500 mb-1">বছর</dt>
                                    <dd class="font-medium text-gray-900" x-text="viewData.year || '-'"></dd>
                                </div>
                                <div>
                            ...
                            <div class="mt-6 pt-5 border-t border-gray-100 flex justify-end gap-3">
                                <button @click="showViewModal = false"
                                    ...
                                    করুন</button>
                                <a :href="viewData.file_url" target="_blank"
                                    class="px-5 py-2.5 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 transition-colors">ফাইল
                                    দেখুন</a>
        ...
        <div x-show="showEditModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div
                class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-slide-up">
                <div class="gradient-logo px-6 py-4 flex items-center justify-between rounded-t-2xl">
                    <h3 class="font-heading font-bold text-white text-lg">ক্যালেন্ডার সম্পাদনা</h3>
                ...
                <div class="p-6">
                    <template x-if="editData">
                        <form :action="'{{ url('admin/academic-calendars') }}/' + editData.id" method="POST"
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">বছর</label>
                                    <input type="text" name="year" x-model="editData.year"
                                        class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                                        placeholder="যেমন: ২০২৬">
                                </div>
                                <div>
                                ...
                                <div class="sm:col-span-2">
                                    <label class="block text-sm font-medium text-gray-700 mb-1">নতুন পিডিএফ ফাইল</label>
                                    <input type="file" name="file" accept="application/pdf"
                                        class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors bg-white">
                                    <p class="mt-1 text-xs text-gray-400">শুধুমাত্র PDF — সর্বোচ্চ 10MB। ফাইল নির্বাচন না করলে পুরাতন থাকবে।</p>
                                    <div class="mt-3 flex items-center gap-3">
                                        <span class="px-2.5 py-1 rounded-lg bg-red-50 text-red-600 text-xs font-semibold">PDF</span>
                                        <a :href="editData.file_url" target="_blank"
                                            class="text-sm text-ris-primary hover:underline" x-text="editData.file_name"></a>
                                    </div>
                                </div>
                                <div class="sm:col-span-2">
                                    <label class="flex items-center gap-2 text-sm font-medium text-gray-700 cursor-pointer">
            ...
            function calendarApp() {
                return {
                    ...
                    async openViewModal(id) {
                        ...
                        this.viewData = null;
                        try {
                            const res = await fetch(`{{ url('admin/academic-calendars') }}/${id}`);
                    ...
                    async openEditModal(id) {
                        ...
                        try {
                            const res = await fetch(`{{ url('admin/academic-calendars') }}/${id}/edit`);
                    ...
                    validateCreateForm(el) {
                        ...
                        const fileInput = el.querySelector('input[name="file"]');
                        if (!fileInput || !fileInput.files.length) {
                            this.createErrors.file = 'পিডিএফ ফাইল আবশ্যক।';
                            valid = false;
                        }
