...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-heading font-bold text-gray-900">নোটিফিকেশন</h1>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="gradient-logo">
                            ...
                            <th class="text-left px-4 py-3.5 font-medium text-white whitespace-nowrap">নোটিফিকেশন</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse($notifications as $index => $notification)
                            ...
                            @endphp
                            <tr
                                class="hover:bg-gray-50 transition-colors {{ $notification->read_at ? '' : 'bg-amber-50/40' }}">
                                <td class="px-4 py-3 text-gray-500 whitespace-nowrap">
                                    {{ ($notifications->currentPage() - 1) * $notifications->perPage() + $index + 1 }}</td>
                                <td class="px-4 py-3">
                                    <div class="flex items-start gap-3">
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium shrink-0 mt-0.5 {{ $typeColors[$type] ?? 'bg-gray-100 text-gray-600' }}">
                                            {{ $typeLabels[$type] ?? 'নোটিফিকেশন' }}
                                        </span>
                                        <div class="min-w-0">
                                            <p class="font-medium text-gray-900">{{ $data['title'] ?? '' }}</p>
                                            <p class="text-gray-500 text-xs mt-0.5 truncate">{{ $data['message'] ?? '' }}
                                            </p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3 text-gray-500 whitespace-nowrap">
                                    {{ $notification->created_at->format('d/m/Y h:i A') }}</td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                    @if ($notification->read_at)
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800">পঠিত</span>
                                    @else
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800">অপঠিত</span>
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-5 py-12 text-center">
                                    ...
                                    <p class="text-gray-500 font-medium">কোনো নোটিফিকেশন নেই</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
