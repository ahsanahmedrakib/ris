...
@section('title', 'নতুন বাস যোগ করুন')
...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-heading font-bold text-gray-900">নতুন বাস যোগ করুন</h1>
            <p class="text-sm text-gray-500 mt-1">নতুন বাসের তথ্য ফর্ম পূরণ করুন</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.transport.store') }}" class="space-y-6">
        @csrf

        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-100">
                <h2 class="font-heading font-semibold text-gray-900 flex items-center gap-2">
                    <svg class="w-5 h-5 text-ris-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h8m0 0v8m0-8l-4 4m4-4l-4 4"/></svg>
                    বাসের তথ্য
                </h2>
            </div>
            <div class="p-5">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">বাস নং <span class="text-red-500">*</span></label>
                        <input type="text" name="bus_no" value="{{ old('bus_no') }}" required
                               ...
                               placeholder="যেমন: ঢাকা-গা-১২-৩৪৫৬">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">রুটের নাম <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">ড্রাইভারের নাম <span class="text-red-500">*</span></label>
                        <input type="text" name="driver_name" value="{{ old('driver_name') }}" required
                               class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                               placeholder="ড্রাইভারের নাম">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">ড্রাইভারের মোবাইল <span class="text-red-500">*</span></label>
                        <input type="text" name="driver_phone" value="{{ old('driver_phone') }}" required
                               class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                               placeholder="01XXXXXXXXX">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">পরিচালকের নাম</label>
                        <input type="text" name="supervisor_name" value="{{ old('supervisor_name') }}"
                               class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                               placeholder="পরিচালকের নাম">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">পরিচালকের মোবাইল</label>
                        <input type="text" name="supervisor_phone" value="{{ old('supervisor_phone') }}"
                               class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors"
                               placeholder="01XXXXXXXXX">
                    </div>
                    <div>
