...
@section('title', 'শ্রেণি তালিকা')

@section('content')
    <div class="space-y-6" x-data="classApp()">

        {{-- Header --}}
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-heading font-bold text-gray-900">শ্রেণি তালিকা</h1>
                <p class="text-sm text-gray-500 mt-1">সকল শ্রেণির তথ্য পরিচালনা করুন</p>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="gradient-logo">
                            ...
                            <th class="text-left px-4 py-3.5 font-medium text-white whitespace-nowrap">শ্রেণির নাম</th>
                            <th class="text-left px-4 py-3.5 font-medium text-white whitespace-nowrap">সেকশন</th>
                            <th class="text-left px-4 py-3.5 font-medium text-white whitespace-nowrap">শিক্ষাবর্ষ</th>
                            <th class="text-left px-4 py-3.5 font-medium text-white whitespace-nowrap">শ্রেণি শিক্ষক</th>
                            <th class="text-left px-4 py-3.5 font-medium text-white whitespace-nowrap">ছাত্র সংখ্যা</th>
                            <th
                    ...
                    <tbody class="divide-y divide-gray-50" data-table-body>
                        @forelse($classes as $index => $class)
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-4 py-3.5 text-gray-500 whitespace-nowrap">{{ $index + 1 }}</td>
                                <td class="px-4 py-3.5 whitespace-nowrap">
                                    <button @click="openViewModal({{ $class->id }})"
                                        class="font-heading font-semibold text-ris-primary hover:underline cursor-pointer">{{ $class->name }}</button>
                                </td>
                                ...
                                <td class="px-4 py-3.5 sticky right-0 bg-white z-10">
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        <button @click="openEditModal({{ $class->id }})"
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-5 py-12 text-center">
                                    ...
                                    <p class="text-gray-500 font-medium">কোনো শ্রেণি নেই</p>
                                    <p class="text-sm text-gray-400 mt-1">নতুন শ্রেণি যোগ করুন</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div x-show="showCreateModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div
                ...
                <div class="gradient-logo px-6 py-4 flex items-center justify-between rounded-t-2xl">
                    <h3 class="font-heading font-bold text-white text-lg">নতুন শ্রেণি যোগ করুন</h3>
                ...
                <form action="{{ route('admin.classes.store') }}" method="POST" class="p-6 space-y-5"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">শ্রেণির নাম <span
                            ...
                            <template x-if="createErrors.name || (createAttempted && !createForm.name)">
                                <p class="mt-1 text-xs text-red-600" x-text="createErrors.name || 'শ্রেণির নাম আবশ্যক।'"></p>
                            </template>
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">শ্রেণি শিক্ষক</label>
                            <select name="class_teacher_id" x-model="createForm.class_teacher_id"
        ...
        <div x-show="showViewModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div
                class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-slide-up">
                <div class="gradient-logo px-6 py-4 flex items-center justify-between rounded-t-2xl">
                    <h3 class="font-heading font-bold text-white text-lg">শ্রেণি তথ্য</h3>
                ...
                <div class="p-6">
                    <template x-if="viewData">
                        <div>
                            <h4 class="font-heading font-bold text-lg text-gray-900 mb-2" x-text="viewData.name"></h4>
                            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 text-sm">
                                ...
                                <div>
                                    ...
                                    <dd class="font-medium text-gray-900" x-text="viewData.academic_year || '-'"></dd>
                                </div>
                                <div>
                                    <dt class="text-gray-500 mb-1">শ্রেণি শিক্ষক</dt>
                                    <dd class="font-medium text-gray-900" x-text="viewData.class_teacher || '-'"></dd>
                                </div>
                                <div>
                                    <dt class="text-gray-500 mb-1">ছাত্র সংখ্যা</dt>
                                    <dd class="font-medium text-gray-900" x-text="viewData.students_count + ' জন'"></dd>
                                </div>
                            </dl>
        ...
        <div x-show="showEditModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div
                class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-slide-up">
                <div class="gradient-logo px-6 py-4 flex items-center justify-between rounded-t-2xl">
                    <h3 class="font-heading font-bold text-white text-lg">শ্রেণি সম্পাদনা</h3>
                ...
                <div class="p-6">
                    <template x-if="editData">
                        <form :action="'{{ url('admin/classes') }}/' + editData.id" method="POST" class="space-y-5"
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <template x-if="editErrors.name || (editAttempted && !editData.name)">
                                        <p class="mt-1 text-xs text-red-600" x-text="editErrors.name || 'শ্রেণির নাম আবশ্যক।'"></p>
                                    </template>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">সেকশন</label>
                                    ...
                                    <select name="class_teacher_id"
                                        ...
                                        @foreach ($teachers as $teacher)
                                            <option :value="'{{ $teacher->id }}'" x-text="'{{ $teacher->name }}'"
                                                :selected="editData.class_teacher_id == '{{ $teacher->id }}'"></option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
            ...
            function classApp() {
                return {
                    ...
                    async openViewModal(id) {
                        ...
                        this.viewData = null;
                        try {
                            const res = await fetch(`{{ url('admin/classes') }}/${id}`);
                    ...
                    async openEditModal(id) {
                        ...
                        try {
                            const res = await fetch(`{{ url('admin/classes') }}/${id}/edit`);
                    ...
                    validateField(field, data, errors) {
                        ...
                        if (field === 'name' && (!val || val.trim() === '')) {
                            errors[field] = 'শ্রেণির নাম আবশ্যক।';
                            return false;
                        }

                        if (field === 'academic_year_id' && (!val || val === '')) {
                            errors[field] = 'শিক্ষাবর্ষ নির্বাচন আবশ্যক।';
                            return false;
                        }
                    ...
                    validateCreateForm(el) {
                        ...
                        this.createErrors = {};
                        let valid = true;
                        ['name', 'academic_year_id'].forEach(f => {
