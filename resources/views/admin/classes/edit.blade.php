...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-heading font-bold text-gray-900">শ্রেণী সম্পাদনা</h1>
            <p class="text-sm text-gray-500 mt-1">শ্রেণীর তথ্য হালনাগাদ করুন</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.classes.update', $class) }}" class="space-y-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-100">
                <h2 class="font-heading font-semibold text-gray-900">শ্রেণীর তথ্য</h2>
            </div>
            <div class="p-5">
                    ...
                    <div>
                        ...
                        <select name="class_teacher_id" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors bg-white">
                            ...
                            @foreach($teachers as $teacher)
                                <option value="{{ $teacher->id }}" {{ old('class_teacher_id', $class->class_teacher_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
