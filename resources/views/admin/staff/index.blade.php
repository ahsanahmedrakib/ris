...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-heading font-bold text-gray-900">কর্মচারী তালিকা</h1>
            <p class="text-sm text-gray-500 mt-1">সকল কর্মচারীদের তথ্য পরিচালনা করুন</p>
        </div>
        <a href="{{ route('admin.staff.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-ris-primary text-white text-sm font-medium rounded-lg hover:bg-ris-dark transition-colors shadow-sm">
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-100">
                        ...
                        <th class="text-left px-5 py-3.5 font-medium text-gray-500">যোগদানের তারিখ</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse(($staff ?? []) as $index => $member)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-5 py-3.5 text-gray-600">{{ $member->phone }}</td>
                            <td class="px-5 py-3.5 text-gray-600">{{ $member->joining_date ? \Carbon\Carbon::parse($member->joining_date)->format('d/m/Y') : '-' }}</td>
                            <td class="px-5 py-3.5">
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('admin.staff.edit', $member) }}" class="p-1.5 rounded-lg text-gray-400 hover:text-amber-600 hover:bg-amber-50 transition-colors" title="সম্পাদনা">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <form method="POST" action="{{ route('admin.staff.destroy', $member) }}" onsubmit="return confirm('আপনি কি নিশ্চিত?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-5 py-12 text-center">
                                ...
                                <p class="text-gray-500 font-medium">কোনো কর্মচারী নেই</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if(isset($staff) && $staff instanceof \Illuminate\Pagination\LengthAwarePaginator && $staff->hasPages())
