...
<div class="space-y-6" x-data="{ saving: false }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <a href="{{ route('admin.attendance.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-ris-primary text-white text-sm font-medium rounded-lg hover:bg-ris-dark transition-colors shadow-sm">
        ...
        <form method="GET" action="{{ route('admin.attendance.index') }}">
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="date" name="date" value="{{ request('date', date('Y-m-d')) }}" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-ris-primary/20 focus:border-ris-primary outline-none transition-colors">
                </div>
                <div>
    ...
    <form method="POST" action="{{ route('admin.attendance.store') }}" x-on:submit="saving = true">
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                ...
                <button type="submit" :disabled="saving" class="inline-flex items-center gap-2 px-4 py-2 bg-ris-primary text-white text-sm font-medium rounded-lg hover:bg-ris-dark transition-colors shadow-sm disabled:opacity-50">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-5 py-3.5 text-center">
                                    <label class="inline-flex items-center justify-center cursor-pointer">
                                        <input type="radio" name="attendance[{{ $student->id }}]" value="present" {{ ($student->today_status ?? '') === 'present' ? 'checked' : '' }} class="w-4 h-4 text-emerald-600 border-gray-300 focus:ring-emerald-500">
                                    </label>
                                </td>
                                <td class="px-5 py-3.5 text-center">
                                    <label class="inline-flex items-center justify-center cursor-pointer">
                                        <input type="radio" name="attendance[{{ $student->id }}]" value="absent" {{ ($student->today_status ?? '') === 'absent' ? 'checked' : '' }} class="w-4 h-4 text-red-600 border-gray-300 focus:ring-red-500">
                                    </label>
                                </td>
                                <td class="px-5 py-3.5 text-center">
                                    <label class="inline-flex items-center justify-center cursor-pointer">
                                        <input type="radio" name="attendance[{{ $student->id }}]" value="late" {{ ($student->today_status ?? '') === 'late' ? 'checked' : '' }} class="w-4 h-4 text-amber-600 border-gray-300 focus:ring-amber-500">
                                    </label>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-5 py-12 text-center text-gray-400 text-sm">এই শ্রেণীতে কোনো ছাত্র নেই</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
