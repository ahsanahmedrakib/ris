                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($buses as $index => $bus)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-5 py-3.5 font-medium text-gray-900">{{ $bus->bus_no ?? $bus->number ?? '-' }}</td>
                            <td class="px-5 py-3.5 text-gray-600">{{ $bus->route ?? '-' }}</td>
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="text-gray-900">{{ $bus->driver->name ?? '-' }}</span>
                                </div>
                            </td>
                            ...
                            <td class="px-5 py-3.5 text-center">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-ris-primary/10 text-ris-primary font-semibold">
                                    {{ $bus->students_count ?? $bus->students->count() ?? 0 }}
                                </span>
                            </td>
                            <td class="px-5 py-3.5 text-center">
