...
@section('content')
    <div class="space-y-6">
        ...
        @if (session('success'))
            <div
                class="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 text-sm text-emerald-800 flex items-center gap-3">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse($structures as $index => $structure)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-5 py-3.5 font-medium text-gray-900">{{ $structure->classRoom->name ?? '-' }}
                                </td>
                                <td class="px-5 py-3.5 text-gray-600">{{ $structure->academicYear->name ?? '-' }}</td>
                                <td class="px-5 py-3.5">
                                    <span
                                        class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-blue-50 text-blue-700">{{ $structure->fee_type }}</span>
                                </td>
                                ...
                                <td class="px-5 py-3.5 text-gray-500 max-w-[240px] truncate">
                                    {{ $structure->description ?? '-' }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="px-5 py-12 text-center text-gray-400 text-sm">কোনো ফি কাঠামো নেই
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
